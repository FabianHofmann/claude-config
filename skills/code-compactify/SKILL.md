...
6. **Clean ripple effects.** A removed branch/return can leave dead vars, unused imports, now-impossible type-hint members (e.g. `| int` after the `return 0` is gone). Remove them. Also **rename locals whose name no longer matches what they hold** — compaction narrows or shifts what a variable represents, and a name like `single_component` that now only tracks whether the *name* is scalar should become `single_name`. A name that lies is a ripple-effect defect, same as a dead var.
...
- **Compute a derived value once, at its definition — don't progressively mutate.** When a flag/intermediate is assigned and then reassigned by anding/oring in a further condition later (`x = a` … `x = x and b`), fold the whole thing into the definition (`x = a and b`). This drops the reassignment, eliminates the window where the variable holds a stale partial value, and often fixes a latent bug where a line *between* the two assignments read the not-yet-final value. Treat any "set, then refine later" sequence as a compaction target.
- **Compaction extends to docstrings and comments, not just code.** Cut prose that restates what the signature, type hints, or a raised error already make explicit — drop "Passing a list to both raises `ValueError`" when the code raises exactly that. Keep the line that conveys *intent or non-obvious usage*; delete the line that narrates mechanics the reader can already see. Verbose docs are the documentation form of needless line spread. (Docs and comments have no test to run, so only trim them riding along with a behavior-preserving code change — not as a standalone rewrite.)
