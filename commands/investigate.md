...
Spawn 4 exploration agents **in a single message** to investigate `$ARGUMENTS` from different angles.
...
```
Map the high-level architecture related to: $ARGUMENTS
...
```
Investigate git history for: $ARGUMENTS

Run these commands and analyze:
- `git log --oneline -30 --all -S '$ARGUMENTS'` (when code was added/changed)
- `git log --oneline -20 --all -- '*$ARGUMENTS*'` (file history)
...
```
Find all tests related to: $ARGUMENTS
...
```
Trace dependencies for: $ARGUMENTS
